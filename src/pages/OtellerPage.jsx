...
import { ArrowLeft, Search, Star, MapPin, ChevronRight, Calendar, Users } from 'lucide-react'
...
export default function OtellerPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center">
        <div className="w-full max-w-[430px] bg-white/90 backdrop-blur-md border-b border-gray-100 px-4 py-3">
          ...
          <div className="flex gap-2 mb-3">
            ...
            <div className="bg-gray-100 rounded-xl px-3 py-2 flex flex-col items-center justify-center">
              <p className="text-gray-400 text-[10px] flex items-center gap-1"><Users size={9} strokeWidth={2} /> Kişi</p>
              <div className="flex items-center gap-2 mt-0.5">
                <button onClick={() => setMisafir(m => Math.max(1, m - 1))} className="text-gray-500 font-bold text-sm w-5 h-5 flex items-center justify-center">−</button>
                <span className="text-gray-900 text-xs font-bold">{misafir}</span>
                <button onClick={() => setMisafir(m => Math.min(8, m + 1))} className="text-gray-500 font-bold text-sm w-5 h-5 flex items-center justify-center">+</button>
              </div>
            </div>
          </div>
      ...
      <div className="pt-[230px] pb-24 px-4 flex flex-col gap-3">
        {liste.map(otel => (
          <button key={otel.id} onClick={() => navigate(`/otel/${otel.id}`, { state: { giris, cikis, misafir } })}
